...
Spectator.describe Hashdiff::LinearCompareArray do
  let(described_class) { Hashdiff::LinearCompareArray }
  ...
  it "finds added items when the old array is empty" do
    ...
    expect(difference).to match to_diff_results [{"+", "[0]", :a}, {"+", "[1]", :b}]
  end
  ...
  it "finds removed items when the new array is empty" do
    ...
    expect(difference).to match to_diff_results [{"-", "[1]", :b}, {"-", "[0]", :a}]
  end
