...
Spectator.describe Hashdiff do
  ...
  it "is able to diff an hash with an empty hash" do
    ...
    expect(diff).to eq(to_diff_results [{"-", "a", 3}, {"-", "b", 2}])

    diff = described_class.diff(b, a)
    expect(diff).to match to_diff_results [{"+", "a", 3}, {"+", "b", 2}]
  end
  ...
  it "is able to diff if mixed key types are removed" do
    ...
    b = Hash(String, String).new
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"-", "a", 1}, {"-", "b", 1}]
  end
  ...
  it "is able to diff changes in hash value which is hash" do
    ...
    expect(diff).to match to_diff_results [{"-", "a.x", 2}, {"-", "a.z", 4}, {"-", "b.x", 3}, {"+", "b.y", 3}, {"~", "b.z", 45, 30}]
  end
  ...
  it "is able to best diff similar objects in array" do
    ...
    expect(diff).to match to_diff_results [{"-", "a[0].x", 2}, {"-", "a[0].z", 4}, {"-", "a[1].y", 22}, {"-", "b.x", 3}, {"-", "b.z", 45}, {"+", "b.y", 22}]
  end
  ...
  it "is able to diff addition of key value pair" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to eq(to_diff_results [{"+", "g", 300}])

    diff = described_class.diff(b, a)
    expect(diff).to match to_diff_results [{"-", "g", 300}]
  end
  ...
  it "is able to diff value type changes" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to eq(to_diff_results [{"~", "a", 3, {"a1" => 1, "a2" => 2}}])

    diff = described_class.diff(b, a)
    expect(diff).to match to_diff_results [{"~", "a", {"a1" => 1, "a2" => 2}, 3}]
  end
  ...
  it "is able to diff value changes: array <=> Array(String).new" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"-", "b[1]", 2}, {"-", "b[0]", 1}]
  end
  ...
  it "is able to diff value changes: array <=> nil" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"~", "b", [1, 2], nil}]
  end
  ...
  it "is able to diff value chagnes: remove array completely" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"-", "b", [1, 2]}]
  end
  ...
  it "is able to diff value changes: remove whole hash" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"-", "b", {"b1" => 1, "b2" => 2}}]
  end
  ...
  it "is able to diff value changes: hash <=> Hash(String, String).new" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"-", "b.b1", 1}, {"-", "b.b2", 2}]
  end
  ...
  it "is able to diff value changes: hash <=> nil" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"~", "b", {"b1" => 1, "b2" => 2}, nil}]
  end
  ...
  it "is able to diff similar objects in array" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"-", "[0].d", 4}, {"+", "[0]", 1}, {"-", "[2]", 3}]
  end
  ...
  it "is able to diff similar & equal objects in array" do
    ...
    diff = described_class.diff(a, b)
    expect(diff).to match to_diff_results [{"-", "[0].d", 4}, {"-", "[1]", {"x" => 5, "y" => 6, "z" => 3}}]
  end
  ...
  context "when :numeric_tolerance requested" do
    it "is able to diff changes in hash value" do
      ...
      expect(diff).to eq(to_diff_results [{"~", "b", 0.0, "hats"}, {"~", "c", 0.65, 0.67}])

      diff = described_class.diff(b, a, numeric_tolerance: 0.01)
      expect(diff).to match to_diff_results [{"~", "b", "hats", 0.0}, {"~", "c", 0.67, 0.65}]
    end
    ...
    it "is able to diff changes in nested values" do
      ...
      diff = described_class.diff(a, b, numeric_tolerance: 0.01)
      expect(diff).to eq(to_diff_results [{"~", "a.x", 0.4, 0.6}, {"-", "b[0]", 13}, {"+", "b[0]", 14}])

      diff = described_class.diff(b, a, numeric_tolerance: 0.01)
      expect(diff).to match to_diff_results [{"~", "a.x", 0.6, 0.4}, {"-", "b[0]", 14}, {"+", "b[0]", 13}]
    end
  end
  ...
  context "when :strip requested" do
    it "strips strings before comparing" do
      ...
      expect(diff).to match to_diff_results [{"~", "b", "fizz buzz", "fizzbuzz"}]
    end
    ...
    it "strips nested strings before comparing" do
      ...
      diff = described_class.diff(a, b, strip: true)
      expect(diff).to match to_diff_results [{"-", "b[0]", "fizz buzz"}, {"+", "b[0]", "fizzbuzz"}]
    end
  end
  ...
  context "when both :strip and :numeric_tolerance requested" do
    it "applies filters to proper object types" do
      ...
      expect(diff).to match to_diff_results [{"~", "d", "baz", 18.5}]
    end
  end
  ...
  context "when :array_path is true" do
    it "returns the diff path in an array rather than a string" do
      ...
      expect(diff).to match to_diff_results [{"~", ["a"], "foo", "bar"}]
    end
    ...
    it "shows array indexes in paths" do
      ...
      expect(diff).to match to_diff_results [{"+", ["a", 3], 3}]
    end
    ...
    it "shows differences with string and symbol keys" do
      ...
      diff = described_class.diff(x, y, array_path: true)
      expect(diff).to match to_diff_results [{"-", ["a"], "foo"}, {"+", [:a], "bar"}]
    end
    ...
    it "supports other key types" do
      ...
      diff = described_class.diff(x, y, array_path: true)
      expect(diff).to match to_diff_results [{"-", [time], "foo"}, {"+", [0], "bar"}]
    end
  end
  ...
  context "when :use_lcs is false" do
    it "shows items in an array as changed" do
      ...
      expect(diff).to match to_diff_results [{"~", "[0]", :a, :c}, {"~", "[1]", :b, :d}]
    end
    ...
    it "shows additions to arrays" do
      ...
      expect(diff).to match to_diff_results [{"+", "a[1]", 1}]
    end
    ...
    it "shows changes to nested arrays" do
      ...
      expect(diff).to match to_diff_results [{"~", "a[0][0]", 0, 1}, {"~", "a[0][1]", 1, 2}]
    end
  end
end
